...
AVL* rechercheAVL(AVL *a,char *id) {
    if (a == NULL){
        return NULL;
	}
    if (strcmp(id, a->identifiant) == 0){
        ...
        return rechercheAVL(a->gauche, id);
        }
    else{
        return rechercheAVL(a->droite, id);
        }
}
...
int main() {
    ...
    Ecrire_fichier(a, f);
    float volumeinit = rechercheAVL(a,"Plant #JA200000I")->valeurreal;
    printf("%f",volumeinit);
    fclose(f);
    fclose(nom);
    fclose(capacite);
